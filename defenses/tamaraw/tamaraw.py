...
def Anoa(list1, list2, parameters):  # inputpacket, outputpacket, parameters
    ...
    if method == 0:
        parameters[0] = (
            "Constant packet rate: " + str(AnoaTime([0, 0])) + ", " + str(AnoaTime([1, 0])) + ". "
        )
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="It simulates tamaraw on a set of web traffic traces."
    )
...
if __name__ == "__main__":
    ...
    for fname in tqdm(flist):
        ...
        with open(os.path.join(args.traces_path, fname), "r") as f:
            # logger.info("Loading from {}".format(os.path.join(args.traces_path, fname)))
            lines = f.readlines()
