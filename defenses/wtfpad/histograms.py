...
class Histogram:
    ...
    def dict_from_list(self, l, num_samples=1000):
        ...
        counts, bins = np.histogram(
            [math.ceil(v) for v in random.sample(l, num_samples)],
    ...
    def dict_from_distr(self, name, params, scale=1.0, num_samples=10000, bin_size=50):
        ...
        if name == "weibull":
            shape = params
            counts, bins = np.histogram(
                np.random.weibull(shape, num_samples) * scale,
            ...
            a, b = params
            counts, bins = np.histogram(
                np.random.beta(a, b, num_samples) * scale,
                bins=self.create_exponential_bins(a=0, b=10, n=bin_size),
            )
            ...
            counts, bins = np.histogram(
                np.random.logistic(location, scale, num_samples),
                bins=self.create_exponential_bins(a=0, b=10, n=bin_size),
            )
            ...
            mu, sigma = params
            counts, bins = np.histogram(
                np.random.lognormal(mu, sigma, num_samples),
                bins=self.create_exponential_bins(a=0, b=10, n=bin_size),
            )
            ...
            shape, scale = params
            counts, bins = np.histogram(
                np.random.gamma(shape, scale, num_samples),
                bins=self.create_exponential_bins(a=0, b=10, n=bin_size),
            )
            ...
            shape, scale = params
            counts, bins = np.histogram(
                genpareto.rvs(shape, scale=scale, size=num_samples),
                bins=self.create_exponential_bins(a=0, b=10, n=bin_size),
            )
    ...
    @classmethod
    def create_exponential_bins(self, sample=None, min_bin=None, a=None, b=None, n=None):
