...
from constants import BURST, GAP, IN, INF, OUT, WAIT
...
class AdaptiveSimulator(object):
    ...
    def load_and_fit(self, histo_fpath, percentile=0.5, fit_distr="norm"):
        ...
        if percentile == 0.5:
            ...
            raise ValueError(
                "Skewing distrib toward longer inter-arrival times makes fake bursts distinguishable from real."
            )
    ...
    def initialize_distributions(self, distributions):
        ...
        for k, v in distributions.items():
            ...
            hist[s][d][on] = histo.new(
                self.init_distrib(k, v), self.interpolate, self.remove_tokens, name=k
            )
        return hist
