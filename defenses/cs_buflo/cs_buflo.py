...
DEFENDED = "../data/defended/cs_buflo"  # TODO: change this path to the path of the defended data.
...
if __name__ == "__main__":
    ...
    if len(sys.argv) < 2:
        print(f"Usage: {sys.argv[1]} <input folder>")
        sys.exit(1)
    ...
    if not os.path.exists(dataset):
        raise FileNotFoundError(f"Path {dataset} not found")
    ...
    print("[#] Simulating CS-BuFlO...")
    print(f"[#] Saving defended traces to {outdirectory}")
